...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupDefaultValues() {
        inputBroker.setText("tcp://172.16.14.56:1883")
        inputClientId.setText("")
        inputTopicBase.setText("home/bedroom/led/led-01")
        inputMessage.setText("ON")
    ...
    private fun publishMessage() {
        val topic = inputTopicBase.text.toString().trim()
        if (topic.isEmpty()) {
            appendLog("Topic is empty")
            return
        }
        val payload = inputMessage.text.toString()
